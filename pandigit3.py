...
""" 素数判定関数 [ミラーラビン判定法] """
def is_prime(digit, twins, number):
    # 順列digitから整数numberへ変換
    number = 0
    for dd in digit:
        number *= 10
        if dd < 10:
           ...
           number += twins
    #print("[is_prime(%d)]" % number)
    # 2であれば素数なので終了
    if number == 2:
        return 1
    # 1もしくは2より大きい偶数であれば素数でないので終了
    if number == 1 or number%2 == 0:
        #print("return false:number=1, even")
        return 0
    ...
    for tn in test_numbers:
        # tn = number -> 任意の自然数kについてtn^k ≡ 0(mod number)なので無視
        if tn == number:
            continue
        # xx ≡ tn^dv(mod number)で初期化
        xx = ipow(tn, dv, number)
        rr = 0
        # tn^dv ≡ 1(mod number)なので無視
        if xx == 1:
            continue
        # rr = 0からssまで順にxx ≡ tn^(2^rrdv) ≡ -1(mod number)を検証
        while xx != mm:
            xx = ipow(xx, 2, number)
            rr += 1
            # xx ≡ 1(mod number) -> xx^2 ≡ 1(mod number)で-1になり得ないので合成数
            if xx == 1 or rr == ss:
...
def main():
    ...
    """ 初期順列の表示 """
    if is_prime(digit, twins, number):
        primcnt += 1
        #print(digit, " ", number)
    """次の順列を生成し、表示を繰り返す """
